...
async def receive_heartbeat(data: HeartbeatData):
    if data.agent_id not in agents:
        print(f"[Router] New agent registered: {data.agent_id} at {data.url}")
        ...
        agents[data.agent_id]["last_heartbeat"] = time.time()
    return {"status": "ok"}
...
async def execute_query(query: str, strategy: str):
    ...
    try:
        ...
        # Mark as unhealthy immediately (circuit breaker)
        agent_data["last_heartbeat"] = 0 
        router_metrics["failed"] += 1
        elapsed = time.time() - start_time
...
async def get_status():
    ...
    agent_list = []
    for agent_id, data in agents.items():
        # Even if dead, we want to return them to the UI so they don't disappear immediately,
        # but we mark them as dead. We don't delete them from the dict until they are really old?
        # Actually our cleanup deletes them. If deleted, they disappear. That's fine.
        pass
        
    # Return all known agents + dead agents that were recently purged
    for agent_id, data in agents.items():
        agent_list.append({
            "id": agent_id,
            "url": data["url"],
...
async def dashboard():
    ...
    <head>
        ...
        <title>Smart Agent — Your Smart Agent</title>
            ...
            <header>
                <div class="brand">
                    ...
                    <div class="brand-text">
                        <h1>Smart Agent</h1>
                        <div class="tagline">Your Smart Agent</div>
                        <div class="sub-tagline">Multiple AI agents working simultaneously to solve every problem</div>
                    </div>
                </div>
                ...
                el.innerHTML = agents.map((a, i) => {
                    ...
                    <div class="agent-card ${status}">
                        ...
                        <div class="agent-info">
                            ...
                            <div class="agent-meta">${esc(a.url)} &nbsp;·&nbsp; ${badge}</div>
                        </div>
