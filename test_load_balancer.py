...
async def main():
    ...
    parser.add_argument("--strategy", type=str, default="least_connections",
                        choices=["least_connections", "round_robin", "random"],
                        help="Routing strategy to use")
    parser.add_argument("--count", type=int, default=15,
                        help="Number of concurrent requests to send")
    args = parser.parse_args()

    print(f"\U0001f680 Sending {args.count} concurrent requests using strategy: '{args.strategy}'...")
    print("-" * 50)
